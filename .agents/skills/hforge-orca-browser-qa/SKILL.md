...
- Run exactly two isolated lanes — exactly one with mobile viewport M (390x844, `hforge-mobile-m`) and exactly one with mobile viewport L (430x932, `hforge-mobile-l`) per `assets/viewport-matrix.json` — with identical bytes from the committed `assets/gym-state-v1-six-month-2026-09-07.json` and identical steps. Read that file as raw bytes and seed the exact UTF-8 string into each isolated profile's raw `gym_state_v1` storage key. Do not invoke the generator or parse/serialize/normalize JSON during a QA run. Viewport application must be proven per lane by eval probe, never by CLI `ok` alone. If the CLI cannot apply a viewport (e.g. `Unknown command: viewport` as observed in Orca 1.4.203), record `blocked_viewport_unapplied` with requested/observed values, page ID, stage, exact command output, and elapsed time, apply only the guide-documented fallback, and never continue as if the viewport were applied. Retain ready tabs for human observation on every terminal outcome.
...
4. Read the committed fixture as raw bytes, verify the recorded identity/hash and storage contract, and seed those exact bytes into each isolated profile. Never generate, parse, serialize, or normalize the fixture during a QA run. Run M/L through navigation, identity eval, fixture verification, one consuming reload, final viewport, and full probe; emit `lanes-started`.
...
- v1.11: consume the committed six-month fixture bytes directly during QA; generation is provenance/maintenance-only, with a bounded exact-byte verification command.

## References

- `references/visual-rubric.md`
- `assets/viewport-matrix.json`
- `assets/gym-state-v1-six-month-2026-09-07.json` (committed QA fixture; read raw)
- `scripts/verify-six-month-fixture.mjs` (maintenance/CI verification; not a per-lane QA step)
- `assets/generate-six-month-fixture.mjs` (provenance/maintenance only; do not invoke during QA)
