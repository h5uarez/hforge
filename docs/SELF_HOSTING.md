...
Hforge is two small containers (a web server and an API) plus a folder of your data.
...
passkey prompt won't appear. To use Hforge from your phone you need a real HTTPS hostname.
...
Hforge only needs the browser to reach it over `https://gym.example.com`.
...
```bash
tar czf hforge-backup-$(date +%F).tar.gz data/
```
