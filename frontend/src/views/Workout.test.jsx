...
describe('scrollable workout composition contracts', () => {
  ...
  it('keeps every set grid inside narrow scrollports with zero x-scroll', () => {
    ...
    expect(css).toContain('.sethead .eff-sp{--eff-toggle-paint-width:clamp(20px,2vw + 12px,28px);position:relative;display:block;min-height:44px')
    expect(css).toContain('.sethead .eff-title{position:absolute;inset:0 calc(var(--eff-toggle-paint-width) + 4px) 0 0')
    expect(css).toContain('.sethead .eff-toggle{position:absolute;right:-8px;top:50%;transform:translateY(-50%);display:flex')
    expect(css).toContain('width:44px;height:44px;min-width:44px')
    expect(css).toContain('.sethead .eff-toggle-surface{display:flex;align-items:center;justify-content:center;width:var(--eff-toggle-paint-width);height:28px')
    ...
    // and the old read-only target region restored beneath matching sets.
    ...
    expect(source).toContain("placeholder: col3 ? '–' : undefined")
