...
describe('scrollable workout composition contracts', () => {
  ...
  it('localizes unilateral markers and keeps prior history visual-only', () => {
    ...
    expect(source).toContain("className={'history-hint' + (col.f === 'w' ? ' weight-history-hint' : col.f === 'r' ? ' reps-history-hint' : '')}")
  ...
  it('scopes the muted placeholder treatment to weight fields in every set layout', () => {
    ...
    expect(css).toContain('.stp .num.reps-input::placeholder,.stp .history-hint.reps-history-hint{color:var(--label);opacity:1}')
