...
describe('scrollable workout composition contracts', () => {
  ...
  it('keeps set completion anchored and lets sheets own completion focus', () => {
    ...
    // silent weight commit or the final workout sheet may take focus after the update.
    expect(toggleBlock?.[0]).not.toContain('focusEntry(')
    // Every weighted completion auto-commits with no TopWeight exception route.
    expect(toggleBlock?.[0]).toContain('if (autoCommitWeight) silentTopWeightCommit(idx)')
    // The silent commit is still one-shot: re-toggling a processed completion cannot advance twice.
    expect(toggleBlock?.[0]).toContain('if (loaded && !e.asked)')
    expect(toggleBlock?.[0]).toContain('e.asked = true')
    expect(toggleBlock?.[0]).not.toContain('topWeightPromptReason')
    expect(toggleBlock?.[0]).not.toContain('topWeightSheet')
  ...
  it('stacks unilateral sides with fluid sub-rows at every width', () => {
    ...
    expect(source).toContain("className={col.f === 'w' ? sideInputClass + ' weight-input' : col.f === 'r' ? sideInputClass + ' reps-input' : sideInputClass}")
  })
  ...
  it('localizes unilateral markers and keeps prior weight history visual-only', () => {
    const css = readFileSync(srcPath('index.css'), 'utf8')
    expect(source).toContain("import { t, sideLabel } from '../lib/i18n.js'")
    expect(source).toContain("const historyHint = col.f === 'w' ? previousSetValue(last, i, col.f) : undefined")
    expect(source).toContain("const historyHint = col.f === 'w' ? previousSetValue(last, i, col.f, side) : undefined")
    expect(source).toContain('configuredRepsForSet(cfg, i)')
    expect(source).toContain("const repTargetForSet = i => configuredRepsForSet(cfg, i) ?? previousSetValue(last, i, 'r') ?? repsForSet(cfg, i)")
    expect(source).toContain('sideReps(repTargetForSet(i))')
    expect(source).toContain("const value = col.f === 'r' && pending && !edited ? ''")
    ...
    expect(source).not.toContain('reps-history-hint')
  ...
  it('scopes the muted placeholder treatment to weight and reps fields in every set layout', () => {
    ...
    expect(source).toContain("className={col.f === 'w' ? 'weight-input' : col.f === 'r' ? 'reps-input' : effortProps.className}")
    expect(source).toContain("className={col.f === 'w' ? sideInputClass + ' weight-input' : col.f === 'r' ? sideInputClass + ' reps-input' : sideInputClass}")
