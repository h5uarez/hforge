...
function MuscleBalance({ S }) {
  ...
  const rated = inWin.some(w => w.entries.some(e => e.sets.some(s => projectSideSet(s).done && isHardSet(projectSideSet(s)))))
  const on = hard && rated
...
export default function Stats() {
  ...
  const exHist = [...new Set(S.workouts.flatMap(w => w.entries.map(e => e.id)))].filter(id => EXIDX[id]).sort((a, b) => exerciseName(EXIDX[a]).localeCompare(exerciseName(EXIDX[b])))
  ...
  return <>
    ...
    <div className="cols">
      ...
      <div className="card exercise-progress">
        <h2>{t('Exercise progress')}</h2>
        {exHist.length ? <>
          <div className="sect-b exercise-progress-selector" style={{ marginBottom: 10 }}>
            <SelectRow className="exercise-progress-row" title={t('Exercise')} sheetTitle={t('Exercise progress')} value={curEx} onChange={setExId}
