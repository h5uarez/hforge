...
function RegisterSheet({ close }) {
  ...
  const go = async () => {
    ...
    } catch (e) { if (e.name !== 'NotAllowedError' && e.name !== 'AbortError') useUI.getState().toast(t(e.message || 'Registration failed')) }
  }
  return <>
  ...
  if (DEMO) return (
    <div className="narrow" style={wrap}>
      ...
      <div className="card small muted" style={{ textAlign: 'left', marginTop: 16 }}>
        {t('This demo runs entirely in your browser on example data — nothing is sent anywhere. Passkey sign-in and sync across your devices come with the Hforge server, which you get by self-hosting it.')}
      </div>
  ...
  return (
    <div className="narrow" style={wrap}>
      ...
      {webauthnOK() ? <>
        ...
        <div style={{ height: 10 }} />
      </> : <div className="card small muted" style={{ textAlign: 'left' }}>{t("This browser doesn't support passkeys — you can still use Hforge locally on this device.")}</div>}
