...
export const loadOfWorkouts = (workouts, pick) =>
  loadOf((workouts || []).flatMap(w =>
    (w.entries || []).map(e => ({ id: e.id, sets: (e.sets || []).map(projectSideSet).filter(s => s.done && (!pick || pick(s))).length }))))
...
export const loadOfActive = active =>
  loadOf((active?.entries || []).map(e => ({ id: e.id, sets: (e.sets || []).map(projectSideSet).filter(s => s.done).length })))
